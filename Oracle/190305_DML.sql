...
-- subquery를 이용한 create    -- dba가 db 테스트 할 때 정도나 사용함. 그냥 이런게 있다고 알아둘 것
...
as
    select *
    from employees
    where 1=0;      ---- 기존 테이블 구조만 가져와서 테이블 생성. 이때, 데이터 타입과 제약조건도 모두 가져오게 됨
...
-- insert
insert into member
values ('정현정', 'kitri', '1234', 31, '8972hj', 'gmail.com', sysdate);    -- 컬럼 순서대로 데이터 입력. 이때, 순서가 달라서 데이터 타입에 위배되거나, 값의 수가 적으면 에러발생함
