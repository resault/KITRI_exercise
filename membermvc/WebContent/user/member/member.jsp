...
function idcheck() {
	...
	if(searchid.length < 5 || searchid.length > 16) {
		resultView.innerHTML = '<font color="gray">아이디는 5자 이상 16자 이하입니다.</font>';
	} else {
	...
	if(httpRequest.readyState == 4) {
		if(httpRequest.status == 200) {
			...
			if(idcount == 0) {
				...
				resultView.innerHTML = '<font color="steelblue">사용중입니다. 다른 아이디를 검색하세요.</font>';
			}
		}
	}else {
...
function register() {
	if(document.getElementById("name").value == "") {
		...
		alert("비밀번호를 입력하세요!!!");
		return;
	} else if(document.getElementById("passcheck").value == "") {
		alert("비밀번호를 확인하세요!!!");
		return;
	} else {
